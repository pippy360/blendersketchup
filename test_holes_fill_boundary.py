...
# We MUST select the new edge to merge it
e3.select = True
v_prev_new.select = True
v_first_new.select = True
...
print("Edges for holes_fill:", len(sel_edges))
bmesh.ops.holes_fill(bm, edges=sel_edges, sides=0)

bm.free()
bm = bmesh.from_edit_mesh(obj.data)
print("Edges after holes_fill:", len(bm.edges))
